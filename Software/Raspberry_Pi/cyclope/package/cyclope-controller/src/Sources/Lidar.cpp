...
namespace Lidar
{
	...
	static int _configureSerialPort()
	{
		...
		{
			LOG(LOG_ERR, "Error : failed to open lidar serial port file (%s).", strerror(errno));
			goto Exit_Error;
		}
		...
		{
			LOG(LOG_ERR, "Error : failed to set serial port reception speed (%s).", strerror(errno));
			goto Exit_Error;
		}
		if (cfsetospeed(&serialPortSettings, B115200) == -1)
		{
			LOG(LOG_ERR, "Error : failed to set serial port transmission speed (%s).", strerror(errno));
			goto Exit_Error;
		}
		...
		{
			LOG(LOG_ERR, "Error : failed to set serial port settings (%s).", strerror(errno));
			goto Exit_Error;
		}
